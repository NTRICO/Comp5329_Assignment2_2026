"""Plot cumulative total-return paths for strategies on the feature-cache test set."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Plot cumulative strategy returns over test time.")
    parser.add_argument(
        "--cache",
    ...
    parser.add_argument(
        ...
        type=int,
        default=20,
        help="Rolling window for display smoothing; set to 1 to plot raw total-return paths.",
    )
    parser.add_argument(
        "--output-png",
        default=str(WORKSPACE_ROOT / "outputs" / "visualizations" / "strategy_returns" / "strategy_returns.png"),
    )
    parser.add_argument(
        "--output-csv",
        default=str(WORKSPACE_ROOT / "outputs" / "visualizations" / "strategy_returns" / "strategy_returns.csv"),
    )
...
def main() -> None:
    ...
    _plot_returns(trace, output_png, asset=asset, smooth_window=max(1, int(args.smooth_window)))

    final_returns = {
        col: float(trace[col].iloc[-1])
        for col in _strategy_columns(trace)
    }
    print("Strategy total-return visualization")
    print("-----------------------------------")
    print(f"asset:           {asset}")
    ...
    print(f"smooth_window:   {max(1, int(args.smooth_window))}")
    print("final total return:")
...
def _build_return_trace(
    *,
    ...
    equity = {result.name: 1.0 for result in results}
    ...
    for result_row, dataset_index in enumerate(test_indices):
        ...
        for t in range(dataset.seq_len):
            ...
            for result in results:
                net_return = float(result.returns.net_returns[result_row, t].item())
                equity[result.name] *= 1.0 + net_return
                row[f"{result.name}_net_return"] = net_return
                row[result.name] = equity[result.name] - 1.0
            rows.append(row)
...
def _plot_returns(trace: pd.DataFrame, output_png: Path, *, asset: str, smooth_window: int) -> None:
    strategy_columns = _strategy_columns(trace)
    ...
    styles = {
        ...
        "buy_hold": {"color": "#111111", "linestyle": "--", "linewidth": 1.8},
        "random_s1234": {"color": "#9c6ade", "linestyle": "-", "linewidth": 1.7, "alpha": 0.9},
        "rolling_ar1_garch": {"color": "#2f80ed", "linestyle": "-", "linewidth": 2.0},
        "policy_encoder_only": {"color": "#d04f3a", "linestyle": "-", "linewidth": 2.2},
    }
    for col in strategy_columns:
        kwargs = styles.get(col, {"linewidth": 1.8})
        y = _smooth_series(trace[col], smooth_window)
        ax.plot(x, y, label=col, **kwargs)

    ax.axhline(0.0, color="#333333", linewidth=1.0, alpha=0.7)
    title = f"Strategy Total Return Over Test Time ({asset})"
    if smooth_window > 1:
        title += f", {smooth_window}-step smoothed"
    ax.set_title(title)
    ax.set_xlabel("time")
...
def _strategy_columns(trace: pd.DataFrame) -> list[str]:
    excluded = {"time_index", "date", "asset", "sequence_rank", "sequence_t", "global_index", "realized_return"}
    return [col for col in trace.columns if col not in excluded and not col.endswith("_net_return")]
