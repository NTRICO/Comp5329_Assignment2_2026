...
from src.fincast_io.cache_builder import build_fincast_distribution_cache


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Build cached FinCast distribution patches for position training.")
    ...
    parser.add_argument("--output", default=str(WORKSPACE_ROOT / "data" / "cache" / "position_fincast_daily_cache.npz"))
...
def main() -> None:
    args = parse_args()
    output = build_fincast_distribution_cache(
        csv_path=args.csv,
    ...
    print(f"saved FinCast cache -> {output}")
