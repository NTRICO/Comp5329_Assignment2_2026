...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="32-stock multi-seed confirmation for gated pre-ASD + LoRA-MoE."
    )
    parser.add_argument(
        "--cache",
        default=str(
    ...
    parser.add_argument(
        ...
        default=str(
            ...
            / "gated_pre_asd_32stock_multiseed"
        ),
    )
    parser.add_argument(
        "--report-path",
        default=str(WORKSPACE_ROOT / "report" / "gated_pre_asd_32stock_multiseed.md"),
    )
...
def write_report(
    *,
    ...
    lines: list[str] = []
    lines.append("# Gated Pre-ASD 32-Stock Multi-Seed Confirmation")
    lines.append("")
    lines.append(
        "This run checks whether `gated_pre_return_asd_lora_moe_patchtst` remains useful "
        "on the larger 32-stock cache across seeds. Training is balanced across "
